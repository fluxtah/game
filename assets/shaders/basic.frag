...
layout(set = 2, binding = 1) uniform sampler2D normalMapSampler;
layout(set = 2, binding = 2) uniform sampler2D metallicRoughnessMapSampler;
...
vec3 calculatePointLight(Light light, vec3 fresnel, vec3 norm, vec3 fragPos, vec3 viewDir, float metallic, float roughness) {
    ...
    float attenuation = calculateAttenuation(light.position.xyz, fragPos,  1.0f, 0.09f, 0.032f);
...
vec3 calculateLight(vec3 norm, vec3 fragPos, vec3 viewDir, float metallic, float roughness, vec3 textureColor) {
    ...
    result += ubo.ambientLightColor.rgb;

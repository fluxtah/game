...
class Emitter(
    ...
    initialSpawnRate: Float = 1.0f,
    initialLifetime: Float = 1.0f,
    ...
    private var _spawnRate: Float = initialSpawnRate
    val spawnRate: Float
        get() {
            return _spawnRate
        }

    private var _lifetime: Float = initialLifetime
    val lifetime: Float
        get() {
            return _lifetime
        }
    ...
    fun setParticleSpawnRate(rate: Float) {
        _spawnRate = rate
        c_setEmitterSpawnRate!!.invoke(handle, rate)
    }

    fun setParticleLifetime(lifetime: Float) {
        _lifetime = lifetime
        c_setEmitterLifetime!!.invoke(handle, lifetime)
    }

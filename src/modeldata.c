...
void readMainModelNode(cgltf_mesh *mesh, ModelData *modelData);
...
cgltf_accessor *
getAttributeAccessorByType(cgltf_mesh *mesh, int primitiveIndex, cgltf_attribute_type type) {
...
AABB readModelAABBNode(cgltf_mesh *mesh) {
    ...
    for (cgltf_size i = 0; i < positionAccessor->count; i++) {
        ...
        aabb.min[0] = (pos[0] < aabb.min[0]) ? pos[0] : aabb.min[0];
        aabb.min[1] = (pos[1] < aabb.min[1]) ? pos[1] : aabb.min[1];
        aabb.min[2] = (pos[2] < aabb.min[2]) ? pos[2] : aabb.min[2];

        aabb.max[0] = (pos[0] > aabb.max[0]) ? pos[0] : aabb.max[0];
        aabb.max[1] = (pos[1] > aabb.max[1]) ? pos[1] : aabb.max[1];
        aabb.max[2] = (pos[2] > aabb.max[2]) ? pos[2] : aabb.max[2];
    }
...
void readModelData(cgltf_data *data, ModelData *modelData) {
    ...
    if (numAABBs > 0) {
        ...
        modelData->aabbs = (AABB *) malloc(numAABBs * sizeof(AABB));
        modelData->num_aabbs = numAABBs;
...
void readMainModelNode(cgltf_mesh *mesh, ModelData *modelData) {
    ...
    cgltf_accessor *positionAccessor = getAttributeAccessorByType(mesh, 0, cgltf_attribute_type_position);
    cgltf_accessor *normalAccessor = getAttributeAccessorByType(mesh, 0, cgltf_attribute_type_normal);
    cgltf_accessor *uvAccessor = getAttributeAccessorByType(mesh, 0, cgltf_attribute_type_texcoord);
    cgltf_accessor *tangentAccessor = getAttributeAccessorByType(mesh, 0, cgltf_attribute_type_tangent);
    ...
    // Images (textures)
    //
    modelData->diffuseImageData = malloc(sizeof(ImageData));
    ...
    modelData->normalMapImageData = malloc(sizeof(ImageData));
    ...
    modelData->metallicRoughnessMapImageData = malloc(sizeof(ImageData));
...
void readImageData(const cgltf_image *image, ImageData *imageData) {
...
void createDefaultImageData(ImageData *modelData, const uint8_t *imageData) {
...
ModelData *loadModelData(const char *filename) {
    ...
    modelData->diffuseImageData = NULL;
    modelData->normalMapImageData = NULL;
    modelData->metallicRoughnessMapImageData = NULL;
    modelData->aabbs = NULL;
    modelData->num_aabbs = 0;

...
VkPipeline createPfxPipeline(
    ...
    VkPipelineShaderStageCreateInfo *shaderStages = createBasicShaderStageInfos(vertShaderModule, fragShaderModule);
    ...
    particleBindingDescription.binding = 1;
    particleBindingDescription.stride = sizeof(Particle);
    particleBindingDescription.inputRate = VK_VERTEX_INPUT_RATE_INSTANCE;

    VkVertexInputBindingDescription bindings[] = {vertexBindingDescription, particleBindingDescription};
    int descriptionCount = 11;
    VkVertexInputAttributeDescription attributeDescriptions[descriptionCount];
    ...
    attributeDescriptions[4].format = VK_FORMAT_R32G32B32A32_SFLOAT; // Format for vec3
    ...
    attributeDescriptions[5].format = VK_FORMAT_R32G32B32A32_SFLOAT; // Format for vec3
    attributeDescriptions[5].offset = offsetof(Particle, initialWorldPos); // Offset of position in Particle struct

    attributeDescriptions[6].binding = 1; // Corresponds to particlePositionBinding
    attributeDescriptions[6].location = 6; // Assuming the next available location index
    attributeDescriptions[6].format = VK_FORMAT_R32G32B32A32_SFLOAT; // Format for vec3
    attributeDescriptions[6].offset = offsetof(Particle, velocity); // Offset of position in Particle struct

    attributeDescriptions[7].binding = 1;
    attributeDescriptions[7].location = 7;
    attributeDescriptions[7].format = VK_FORMAT_R32G32B32A32_SFLOAT;
    attributeDescriptions[7].offset = offsetof(Particle, scale);

    attributeDescriptions[8].binding = 1;
    attributeDescriptions[8].location = 8;
    attributeDescriptions[8].format = VK_FORMAT_R32G32B32A32_SFLOAT;
    attributeDescriptions[8].offset = offsetof(Particle, color);

    attributeDescriptions[9].binding = 1;
    attributeDescriptions[9].location = 9;
    attributeDescriptions[9].format = VK_FORMAT_R32_SFLOAT;
    attributeDescriptions[9].offset = offsetof(Particle, lifeTime);
    ...
    attributeDescriptions[10].offset = offsetof(Particle, spawnTime);

    VkPipelineVertexInputStateCreateInfo vertexInputInfo = {};
    vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
    vertexInputInfo.vertexBindingDescriptionCount = 2;
    vertexInputInfo.pVertexBindingDescriptions = bindings; // Pointer to binding descriptions
    vertexInputInfo.vertexAttributeDescriptionCount = descriptionCount;

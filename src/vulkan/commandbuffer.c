...
void calculateOBBTransform(Entity *entity, int obbIndex, mat4 transform) {
    // Start with the OBB's transformation matrix
    memcpy(transform, entity->obbs[obbIndex].transform, sizeof(mat4));

    // Scale the unit cube to match the full dimensions of the AABB
    vec3 fullExtents;
    glm_vec3_scale(entity->obbs[obbIndex].extents, 2.0f, fullExtents); // Scale the extents by 2
...
void recordDebugCommandBuffer(
    ...
    for (size_t i = 0; i < ktEntities->size; ++i) {
        ...
        if (entity->useOBB) {
            for (int j = 0; j < entity->num_obbs; j++) {
                calculateOBBTransform(entity, j, transform);
            ...
            for (int j = 0; j < entity->num_aabbs; j++) {
                calculateAABBTransform(entity, j, transform);

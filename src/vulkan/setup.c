...
ApplicationContext *createApplication() {
    ...
    applicationContext->spritePipelineConfig = createSpriteShaderPipelineConfig(
            vulkanDeviceContext,
            applicationContext->commandPool,
            vulkanSwapchainContext
    );
    if (applicationContext->spritePipelineConfig == NULL) {
        LOG_ERROR("Failed to create sprite shader pipeline config");
        destroyApplication(applicationContext);
        return NULL;
    }
...
void destroyApplication(ApplicationContext *context) {
    ...
    if (context->renderPass != VK_NULL_HANDLE && context->vulkanDeviceContext != NULL) {
    ...
    if (context->spritePipelineConfig != NULL) {
        destroyPipelineConfig(context->vulkanDeviceContext, context->spritePipelineConfig);
    }
